...
- ASS/SSA subtitles are supported. Serve the original file with a `.ass` or `.ssa` extension in the `url`, or make sure the file starts with the standard ASS sections (e.g. `[Script Info]`). When the user enables "Style ASS subtitles" in the player settings, Stremio renders the file with its own fonts, colours and positioning. Do not wrap ASS files in the `subtitles.vtt?from=` URL above, as converting them to VTT drops the styling
